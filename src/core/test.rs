...
fn simulated_pair() -> SimulatedHardware {
    SimulatedHardware::new(vec![client_addr(), server_addr()])
}
...
/// handshake, and then one packet each way.
#[test]
fn test_simple() {
    debug_logging();
    let mut simulated_hardware = simulated_pair();
    ...
    let mut cores = BTreeMap::from([
        (client_addr(), core::ConcreteCore::Client(client_core)),
        (server_addr(), core::ConcreteCore::Server(server_core)),
    ]);
    ...
    // long enough that it can actually read the outgoing packet.
